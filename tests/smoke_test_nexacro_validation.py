...
def main() -> None:
    ...
    nexacro = build_nexacro_plan(analysis, backend)
    report = build_validation_report(analysis, backend_plan=backend, nexacro_plan=nexacro, frontend_key='nexacro')
    ...
    Path('tests/smoke_test_nexacro_validation_output.json').write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding='utf-8')
